...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run ELECTS Early Classification training on the BavarianCrops dataset."
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="trade-off parameter of earliness and accuracy (eq 6): "
    ...
    parser.add_argument(
        ...
        type=float,
        default=10,
        help="additive smoothing parameter that helps the "
    ...
    parser.add_argument(
        ...
        choices=["cuda", "cpu"],
        help="'cuda' (GPU) or 'cpu' device to run the code. "
    ...
    parser.add_argument(
        ...
        type=int,
        default=70,
        help="sequencelength of the time series. If samples are shorter, "
    ...
    parser.add_argument(
        "--dataroot",
        type=str,
        default=os.path.join(os.path.expanduser("~"), "elects_data"),
...
def main(args):
    ...
    testdataloader = DataLoader(test_ds, batch_size=args.batchsize)
    ...
    optimizer = torch.optim.AdamW(
        [
            {"params": no_decay, "weight_decay": 0, "lr": args.learning_rate},
            {"params": decay},
        ],
    ...
    if args.resume and os.path.exists(args.snapshot):
        ...
        optimizer.load_state_dict(
            torch.load(optimizer_snapshot, map_location=args.device)
        )
        ...
        for epoch in pbar:
            ...
            if len(df) > 2:
                if testloss < df.testloss[:-1].values.min():
                    ...
                    optimizer_snapshot = os.path.join(
                        os.path.dirname(args.snapshot),
                        os.path.basename(args.snapshot).replace(
                            ".pth", "_optimizer.pth"
                        ),
                    )
            ...
            if args.patience is not None:
                if not_improved > args.patience:
                    print(
                        f"stopping training. testloss {testloss:.2f} did not improve in {args.patience} epochs."
                    )
                    break
